...
test('production build keeps OCR hard block Table-first PDF Tools and adds performance runtime', async () => {
  const build = await read('build.mjs');
  assert.match(build, /book-ocr-browser-performance\.mjs/);
  assert.match(build, /ripscan-pwa-v5\.0\.0/);
  ...
  assert.match(build, /performance-runtime\.mjs/);
  assert.match(build, /performance-image-worker\.js/);
  assert.match(build, /studio-virtualization\.mjs/);
});

test('syntax check includes existing pipelines and new performance modules', async () => {
  ...
  for (const required of [
    ...
    'roundtrip-export.mjs', 'pdf-tools-ui.js', 'performance-runtime.mjs',
    'performance-bootstrap.js', 'performance-image-worker.js', 'performance-image-client.mjs',
