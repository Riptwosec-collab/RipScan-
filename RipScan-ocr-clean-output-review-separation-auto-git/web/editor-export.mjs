import { getTableCell } from './document-model.mjs';
import {
  DEFAULT_OUTPUT_MODE,
  OUTPUT_MODES,
...
export function normalizeExportOptions(options = {}) {
  ...
  return {
    ...
    outputMode: options.outputMode || options.mode || DEFAULT_OUTPUT_MODE,
    includeReviewMetadata: Boolean(options.includeReviewMetadata),
...
function reviewItemFromElement(element, text) {
  const status = element.dataset.reviewStatus || element.dataset.verifiedStatus || element.dataset.status || 'verified';
  return {
    type: 'text',
    text,
    status,
    reviewStatus: status,
    confidence: Number(element.dataset.confidence || 1),
    includeInExport: element.dataset.includeInExport === '' ? undefined : element.dataset.includeInExport === 'true',
    confirmed: element.dataset.confirmed === 'true' || element.dataset.reviewed === 'true',
    reviewed: element.dataset.reviewed === 'true',
    userOverride: element.dataset.exportOverride === 'true',
  };
}

function prepareElementForCleanRender(root, options = {}) {
  const changes = [];
  const reviewElements = root.querySelectorAll?.('[data-review-status],[data-verified-status],[data-status]') || [];
  for (const element of reviewElements) {
    const item = reviewItemFromElement(element, element.textContent || '');
    if (!shouldIncludeInExport(item, { mode: options.outputMode })) {
      changes.push({ type: 'style', element, value: element.style.visibility });
  ...
  while (node) {
    const clean = sanitizeTextForExport(node.nodeValue || '', { trim: false });
    if (clean !== node.nodeValue) {
      changes.push({ type: 'text', node, value: node.nodeValue });
  ...
  return () => {
    for (const change of changes.reverse()) {
      if (change.type === 'text') change.node.nodeValue = change.value;
      else change.element.style.visibility = change.value;
    }
  };
}

export function getExportPreview(documentModel, options = {}) {
  return buildExportPreview(documentModel, normalizeExportOptions(options));
}
...
export async function exportPageElements(elements, pages, options = {}, filename = 'ripscan-document', onProgress = () => {}) {
  ...
  const cleanModel = documentModel ? sanitizeDocumentModelForExport(documentModel, normalized) : null;
  const selectedCleanPages = cleanModel ? selected.map(index => cleanModel.pages[index]).filter(Boolean) : [];

  if (normalized.format === 'searchable-pdf') return printSearchableDocument(selectedCleanPages.length ? selectedCleanPages : pairs.map(item => item.page), filename, normalized);
  if (normalized.format === 'json') {
    if (!documentModel) throw new Error('ไม่พบ Document Model สำหรับ JSON');
    const payload = buildJsonExportPayload(documentModel, normalized);
    return downloadBlob(new Blob([JSON.stringify(payload, null, 2)], { type: 'application/json;charset=utf-8' }), `${base}.json`);
  }
  if (normalized.format === 'txt') {
    if (!documentModel) throw new Error('ไม่พบ Document Model สำหรับ TXT');
    return downloadBlob(new Blob([buildCleanExportText(documentModel, normalized)], { type: 'text/plain;charset=utf-8' }), `${base}.txt`);
  }
  if (normalized.format === 'docx') {
    if (!documentModel) throw new Error('ไม่พบ Document Model สำหรับ DOCX');
    return downloadBlob(await modelToDocxBlob(documentModel, normalized), `${base}.docx`);
  }
  if (normalized.format === 'xlsx') {
    if (!documentModel) throw new Error('ไม่พบ Document Model สำหรับ XLSX');
    return downloadBlob(await modelToXlsxBlob(documentModel, normalized), `${base}.xlsx`);
  }
  if (normalized.format === 'pdf') {
    ...
    for (let position = 0; position < pairs.length; position += 1) {
      ...
      const restore = prepareElementForCleanRender(element, normalized);
      let canvas;
      try { canvas = await renderElementToCanvas(element, page, normalized); }
      finally { restore(); }
...
export async function modelToDocxBlob(documentModel, options = {}) {
  ...
  const model = sanitizeDocumentModelForExport(documentModel, options);
  const zip = new globalThis.JSZip();
...
export async function modelToXlsxBlob(documentModel, options = {}) {
  ...
  for (const [pageIndex, page] of (model.pages || []).entries()) {
    ...
    if (!tables.length) {
      const rows = (page.blocks || []).filter(block => !block.hidden && ['text', 'header', 'footer', 'field'].includes(block.type)).sort((a, b) => a.y - b.y || a.x - b.x).map(block => [block.type === 'field' ? block.label : '', block.type === 'field' ? block.value : block.text || '']);
