  ...
  dialog.innerHTML = `
    <div class="convert-center-card">
      ...
      <div class="convert-grid">
        ...
        <section><h3>รูปแบบผลลัพธ์</h3><label>แปลงเป็น<select id="convertFormat"><option value="pdf">PDF</option><option value="searchable-pdf">Searchable PDF (ผ่าน Print)</option><option value="png">PNG</option><option value="jpg">JPG</option><option value="docx">DOCX</option><option value="xlsx">XLSX</option><option value="txt">TXT</option><option value="json">JSON Structured</option></select></label><label>โหมด Office<select id="convertOfficeMode"><option value="editable">แก้ไขข้อความ ตาราง และเซลล์ได้ (แนะนำ)</option><option value="original">เหมือนต้นฉบับ (Word: ภาพ + ข้อความค้นหา)</option></select></label><label>ขนาดหน้า<select id="convertPageSize"><option value="source">ตามต้นฉบับ</option><option value="A4">A4</option><option value="A5">A5</option><option value="Letter">Letter</option><option value="Legal">Legal</option><option value="custom">กำหนดเอง</option></select></label><label>แนวกระดาษ<select id="convertOrientation"><option value="portrait">แนวตั้ง</option><option value="landscape">แนวนอน</option></select></label></section>
...
function conversionOptions() {
  ...
  return normalizeExportOptions({
    ...
    officeMode: $('#convertOfficeMode').value,
    selectedPages,
  });
}
