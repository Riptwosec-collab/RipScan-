...
test('Document Studio exposes visual and structured editable views', async () => {
  ...
  for (const required of [
    'Document Studio',
    'data-studio-view="visual"',
    'data-studio-view="structure"',
    'contenteditable',
    'studio-block-handle',
...
test('Convert Center supports PDF JPG PNG resize quality and page options', async () => {
  const source = await read('web/document-studio.js');
  for (const required of [
    'Convert Center',
    'searchable-pdf',
...
test('Document Studio CSS provides three-panel WYSIWYG layout and responsive UI', async () => {
  ...
  for (const required of [
    ...
    '.studio-editable-table',
    '.convert-center-shell',
...
test('production build injects and caches Document Studio v3 assets', async () => {
  ...
  for (const required of ['document-model.mjs', 'office-import.mjs', 'editor-export.mjs', 'document-studio.js']) assert.ok(packageJson.scripts.check.includes(required), `missing check ${required}`);
});
